...
func (b *Broker[T]) Publish(ctx context.Context, topic string, msg T) (int, error) {
	...
	// Snapshot the subscriber set so we hold the lock briefly and never invoke
	// channel sends while holding it (a slow path under contention).
	subs := b.topics[topic]
	targets := make([]*subscription[T], 0, len(subs))
	for sub := range subs {
