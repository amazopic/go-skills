---
name: structural-flyweight
description: Flyweight — stub. Catalog placeholder; body to be filled in a later iteration. See SUMMARY at https://github.com/tmrts/go-patterns and go-patterns-2 archive for source material.
category: structural
status: stub
---

# Flyweight

> **Status:** stub — body not yet written. This entry exists so the catalog and index remain complete.
>
> **Sources to mine when filling:**
> - `go-old-pattern/go-patterns-1/SUMMARY.md` (taxonomy)
> - `go-old-pattern/go-patterns-2/Structural/Flyweight/` (if present — runnable code + README)
> - Public references: <a href="https://github.com/tmrts/go-patterns">tmrts/go-patterns</a>, <a href="https://github.com/AlexanderGrom/go-patterns">AlexanderGrom/go-patterns</a>
> **Existing example:** see `examples/structural/flyweight/`
