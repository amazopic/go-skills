...
func (c *Coroutine[T]) Next() (T, bool) {
    v, ok := <-c.yield
    if ok {
        // Wake the body — unless it just exited on its own (e.g. ctx cancelled
        // while it sat in the second select, so it returned via ctx.Done()
        // instead of receiving resume). On exit it closes c.yield, so observe
        // that here too; otherwise Next blocks forever on a resume nobody reads.
        select {
